...
public class FlowTaskLinkController {
    ...
    public Result<List<TaskLinkVO>> listBySource(@RequestBody(required = false) Map<String, Object> body) {
        Map<String, Object> p = body == null ? new HashMap<>() : body;
        return Result.success("获取成功", flowTaskLinkService.listBySource(
                ParamUtil.str(p.get("taskId")), ParamUtil.str(p.get("dispatchId")), ParamUtil.str(p.get("periodId"))));
    }

    /** 按目标反查（收到任务侧查「被哪些期次关联」传 taskId；dispatchId/periodId 为兼容保留） */
    @PostMapping("/by-target")
    public Result<List<TaskLinkVO>> listByTarget(@RequestBody(required = false) Map<String, Object> body) {
        Map<String, Object> p = body == null ? new HashMap<>() : body;
        return Result.success("获取成功", flowTaskLinkService.listByTarget(
                ParamUtil.str(p.get("taskId")), ParamUtil.str(p.get("dispatchId")), ParamUtil.str(p.get("periodId"))));
    }
