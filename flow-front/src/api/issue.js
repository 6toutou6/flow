...
export function getIssueList(params) {
  const { status, department, priority, column1, column2, column3, column4, column5, column6, page = 1, limit = 10 } = params
...
export function getIssue(id) {
  const issue = issueData.find(item => item.id === id)
  if (issue) {
    return createMockResponse({
      ...issue,
...
export function createIssue(data) {
  const newIssue = {
    id: Mock.mock('@id'),
    issueNo: '#RECT-' + new Date().getFullYear() + '-' + String(issueData.length + 1).padStart(3, '0'),
    ...data,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    statusText: getStatusText(data.status || 'pending'),
    priorityText: getPriorityText(data.priority || 'medium')
  }
  issueData.unshift(newIssue)
  return createMockResponse(newIssue, '创建成功')
}

export function updateIssue(id, data) {
  const index = issueData.findIndex(item => item.id === id)
  if (index !== -1) {
    issueData[index] = {
      ...issueData[index],
      ...data,
    ...
    return createMockResponse(issueData[index], '更新成功')
  }
  return Promise.reject({
    code: 50000,
    message: '问题不存在'
...
export function getIssueStats() {
  const stats = {
    ...
    pending: issueData.filter(i => i.status === 'pending').length,
    rectifying: issueData.filter(i => i.status === 'rectifying').length,
    overdue: issueData.filter(i => i.status === 'overdue').length,
