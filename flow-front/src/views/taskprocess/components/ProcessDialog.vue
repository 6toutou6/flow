        ...
        <div v-if="flowChain.length > 0" class="chain-section">
          ...
          <div class="chain-track-h">
            <template v-for="(item, idx) in flowChain">
              <div
                ...
                :title="`${idx + 1}. ${item.nodeName}（${statusLabel(item.status)}）${item.assignedText ? '\n处理人：' + item.assignedText : ''}${item.taskNode && item.taskNode.submitStatus === 1 && item.assignedText && item.assignedText !== (item.taskNode.handlerName + (item.taskNode.handlerUserId ? ' ' + item.taskNode.handlerUserId : '')) ? '\n实际处理：' + item.taskNode.handlerName + (item.taskNode.handlerUserId ? ' ' + item.taskNode.handlerUserId : '') : ''}`"
          ...
          <div v-if="expandedItem" class="step-detail" @click.stop>
            <div class="sd-head">
              ...
              <span v-if="expandedItem.taskNode && expandedItem.taskNode.submitStatus === 1 && expandedItem.taskNode.handleTime" class="step-time"><i class="el-icon-time" /> {{ expandedItem.taskNode.handleTime }}</span>
              ...
              <div class="sd-handlers">
                <template v-if="expandedItem.taskNode && expandedItem.taskNode.submitStatus === 1">
                  <span class="actual-handler"><i class="el-icon-user" /> 实际处理：{{ expandedItem.taskNode.handlerName }}{{ expandedItem.taskNode.handlerUserId ? ' ' + expandedItem.taskNode.handlerUserId : '' }}</span>
                  <span v-if="expandedItem.assignedText && expandedItem.assignedText !== (expandedItem.taskNode.handlerName + (expandedItem.taskNode.handlerUserId ? ' ' + expandedItem.taskNode.handlerUserId : ''))" class="sd-meta"><i class="el-icon-s-custom" /> 当时分配：{{ expandedItem.assignedText }}</span>
                ...
                <template v-else-if="expandedItem.taskNode && expandedItem.taskNode.submitStatus === 0">
                  <span class="sd-meta"><i class="el-icon-user" /> 处理人：{{ expandedItem.assignedText || expandedItem.pendingNames || expandedItem.taskNode.handlerName || '—' }}</span>
            ...
            <template v-else>
              ...
              <div v-if="expandedItem.taskNode && expandedItem.taskNode.baseDataList && expandedItem.taskNode.baseDataList.length > 0" class="bd-block">
                <div class="bd-title">
                  ...
                  <span class="bd-handler-note"><i class="el-icon-user" /> 「{{ expandedItem.nodeName }}」节点由 {{ expandedItem.taskNode.handlerName }}{{ expandedItem.taskNode.handlerUserId ? ' ' + expandedItem.taskNode.handlerUserId : '' }} 填写</span>
                </div>
          ...
          <div v-else class="chain-track-v">
            ...
            >
              <div class="step-head">
                ...
                <span v-if="item.taskNode && item.taskNode.submitStatus === 1 && item.taskNode.handleTime" class="step-time"><i class="el-icon-time" /> {{ item.taskNode.handleTime }}</span>
              ...
              <div class="step-meta">
                <template v-if="item.taskNode && item.taskNode.submitStatus === 1">
                  <span class="actual-handler"><i class="el-icon-user" /> 实际处理：{{ item.taskNode.handlerName }}{{ item.taskNode.handlerUserId ? ' ' + item.taskNode.handlerUserId : '' }}</span>
                  <span v-if="item.assignedText && item.assignedText !== (item.taskNode.handlerName + (item.taskNode.handlerUserId ? ' ' + item.taskNode.handlerUserId : ''))"><i class="el-icon-s-custom" /> 当时分配：{{ item.assignedText }}</span>
                ...
                <template v-else-if="item.taskNode && item.taskNode.submitStatus === 0">
                  <span><i class="el-icon-user" /> 处理人：{{ item.assignedText || item.pendingNames || item.taskNode.handlerName || '—' }}</span>
              ...
              <div v-if="vExpandedNodeIds.includes(item.nodeId)" class="step-expanded" @click.stop>
                ...
                <template v-else>
                  ...
                  <div v-if="item.taskNode && item.taskNode.baseDataList && item.taskNode.baseDataList.length > 0" class="bd-block">
                    <div class="bd-title">
                      <i class="el-icon-collection" /> 任务基础信息
                      <span class="bd-handler-note"><i class="el-icon-user" /> 「{{ item.nodeName }}」节点由 {{ item.taskNode.handlerName }}{{ item.taskNode.handlerUserId ? ' ' + item.taskNode.handlerUserId : '' }} 填写</span>
                    </div>
...
export default {
  ...
  computed: {
    ...
    isRefill() {
      ...
      const curNodeId = this.detail && this.detail.task ? this.detail.task.currentNodeId : null
    ...
    flowChain() {
      ...
      return tplNodes.map(tpl => {
        ...
        // 节点待办处理人（同一节点多处理人全部展示，带用户号）
        const pendingNames = formatNodeHandlers(nodes.filter(n => n.submitStatus === 0), false)
        // 节点当时分配的全部处理人（已完成节点回看也能看到当初分配了谁）
        const assignedText = formatNodeHandlers(nodes, false)
        return { nodeId: tpl.id, nodeName: tpl.nodeName, nodeType: tpl.nodeType, sortNum: tpl.sortNum, taskNode: rep, pendingNames, assignedText, status, guideText: tpl.guideText, guideFiles: tpl.guideFiles }
      })
    },
    ...
    taskFinished() {
      return this.detail && this.detail.task && this.detail.task.status === '已结束'
    },
...
.pd-right { flex: 1; min-width: 0; overflow-y: auto; background: #fff; border: 1px solid $border; border-radius: 3px; padding: 20px; }
.info-section, .chain-section, .form-section, .next-section { background: #fff; border: 1px solid $border; border-radius: 3px; padding: 16px 18px; }
...
.chain-chip { display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 2px; font-size: 13px; font-weight: 600; flex-shrink: 0; line-height: 1.5; cursor: default; border: 1px solid transparent;
  &.chip-done { background: #15803D; color: #fff; }
  &.chip-current { background: rgba(var(--color-primary-rgb),0.14); color: $primary; border-color: rgba(var(--color-primary-rgb),0.4); }
  &.chip-rejected { background: rgba(180, 83, 9,0.16); color: #B45309; border-color: rgba(180, 83, 9,0.45); }
  &.chip-pending { background: #f0f0f0; color: #aaa; }
  &.clickable { cursor: pointer;
    &:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.12); }
  }
  &.expanded { box-shadow: 0 0 0 2px rgba(var(--color-primary-rgb),0.25); }
}
...
.chip-current .cc-no { background: rgba(var(--color-primary-rgb),0.2); }
...
.meta-reason { color: #B45309; background: rgba(180, 83, 9,0.1); border-radius: 4px; padding: 2px 8px; max-width: 340px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: inline-flex; align-items: center; gap: 3px; }
.meta-pass { color: #15803D; background: rgba(21, 128, 61,0.08); border-radius: 4px; padding: 2px 8px; max-width: 340px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: inline-flex; align-items: center; gap: 3px; }
// 展开的节点详情
.step-detail { margin-top: 12px; padding: 14px 16px; background: #fff; border: 1px dashed $border; border-radius: 3px;
  .sd-head { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 10px; }
  .sd-no { width: 22px; height: 22px; border-radius: 50%; background: #15803D; color: #fff; display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; flex-shrink: 0; }
...
// 横/竖切换按钮（对齐流程详情）
.orient-toggle { display: inline-flex; align-items: center; gap: 2px; margin-left: auto; background: #f5f5f5; border: 1px solid #e8e8e8; border-radius: 2px; padding: 2px; }
.orient-btn { display: inline-flex; align-items: center; gap: 3px; padding: 3px 10px; border-radius: 2px; font-size: 11px; color: #757575; cursor: pointer; transition: all .2s; user-select: none;
  &:hover { color: $primary; }
...
.step-no { width: 22px; height: 22px; border-radius: 50%; background: #CBD5E1; color: var(--color-primary); display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; flex-shrink: 0; }
...
.step-expanded { margin-top: 12px; padding: 12px; background: #fff; border-radius: 3px; border: 1px solid rgba(var(--color-primary-rgb),0.25); overflow: hidden;
  .sd-head { margin-bottom: 10px; padding: 8px 10px; border-radius: 2px; background: var(--color-primary-surface); border: 1px solid #E2E8F0; }
}
.st-done .step-expanded .sd-no { background: #15803D; }
.st-rejected .step-expanded .sd-no { background: #B45309; }
.st-current .step-expanded .sd-no { background: $primary; }
.bd-block { margin-top: 10px; padding-top: 8px; border-top: 1px dashed #CBD5E1; }
.bd-title { font-size: 13px; font-weight: 700; color: #B45309; margin-bottom: 2px; display: flex; align-items: center; gap: 4px; }
...
.bd-form-section { background: #EFF6FF; }
...
// 节点填写说明（处理端展示）
.guide-section { background: #EFF6FF; border: 1px solid rgba(180, 83, 9,0.35); border-radius: 3px; padding: 14px 18px; }
...
.node-guide { margin-bottom: 10px; padding: 10px 12px; background: #EFF6FF; border: 1px dashed rgba(180, 83, 9,0.4); border-radius: 2px;
  .guide-text { border: none; padding: 0 0 4px; background: transparent; }
}
// 折叠面板：并入当前节点卡片，可展开/收回
.guide-fold { margin-bottom: 14px; border: 1px dashed rgba(180, 83, 9,0.4); border-radius: 3px; background: #EFF6FF; overflow: hidden;
...
.guide-file-tag { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; background: #fff; border: 1px solid #CBD5E1; border-radius: 4px; font-size: 12px; color: var(--color-primary-hover);
  i { color: #B45309; }
}

// 完成节点提示
.end-tip { display: flex; align-items: center; gap: 8px; padding: 14px 16px; background: rgba(21, 128, 61,0.08); border-radius: 3px; color: #15803D; font-size: 14px;
  i { font-size: 18px; }
}
.dialog-footer { text-align: right; }
.readonly-tip { display: inline-flex; align-items: center; gap: 5px; margin-right: 12px; font-size: 13px; color: #15803D; background: rgba(21, 128, 61,0.08); padding: 6px 12px; border-radius: 2px; }
