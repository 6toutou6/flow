...
CREATE TABLE `flow_template_node` (
  ...
  `branch_config` text COMMENT '条件分支配置 JSON {"branches":[{"fieldKey":"","op":"","value":"","targetNodeId":""}],"defaultNodeId":""}；为空则按 sort_num 顺序流转',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `flow_template_field` (
  ...
  `field_type` varchar(30) NOT NULL COMMENT '字段类型：text单行、textarea多行、number数字、date日期、radio单选、checkbox多选、file文件、image图片、user人员、dept部门',
  `sort_num` int NOT NULL DEFAULT '0' COMMENT '排序号',
  ...
  `visible_when` varchar(500) DEFAULT NULL COMMENT '字段显隐条件 JSON {"fieldKey":"","op":"","value":""}；为空恒显示',
  `editable_when` varchar(500) DEFAULT NULL COMMENT '字段只读条件 JSON {"fieldKey":"","op":"","value":""}；为空恒可编辑',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `flow_task_dispatch_node` (
  ...
  `branch_config` text COMMENT '条件分支配置快照 JSON',
...
CREATE TABLE `flow_task_node` (
  ...
  `transfer_from_user_id` varchar(64) DEFAULT NULL COMMENT '转办原处理人用户号（被转办时记录）',
  `transfer_from_user_name` varchar(50) DEFAULT NULL COMMENT '转办原处理人姓名',
  PRIMARY KEY (`id`),
